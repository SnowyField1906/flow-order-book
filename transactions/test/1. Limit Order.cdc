        ...
        if isBid {
          ...
          // exists a matching ask order
            ...
            if OrderBookV14.askOffers[price]!.amount > amount {
              ...
              let contractFusdVault <- FlowFusdVaultV4.withdrawFusd(amount: amount, owner: OrderBookV14.askOffers[price]!.maker)
              ...
              if OrderBookV14.askOffers[price]!.amount < amount {
                ...
                // transfer FUSD from contract's to this order
                let contractFusdVault <- FlowFusdVaultV4.withdrawFusd(amount: OrderBookV14.askOffers[price]!.amount, owner: OrderBookV14.askOffers[price]!.maker)
                let userFusdVault = getAccount(self.maker).getCapability(/public/fusdReceiver)
                  .borrow<&{FungibleToken.Receiver}>()!
                userFusdVault.deposit(from: <- contractFusdVault)
        ...
        // is selling / ask
          ...
          // exists a matching bid order
            ...
            if OrderBookV14.bidOffers[price]!.amount > amount {
              ...
              let contractFlowVault <- FlowFusdVaultV4.withdrawFlow(amount: amount*price, owner: OrderBookV14.bidOffers[price]!.maker)
              ...
              if OrderBookV14.bidOffers[price]!.amount < amount {
                ...
                // transfer Flow from contract's to this order
                let contractFlowVault <- FlowFusdVaultV4.withdrawFlow(amount: price*OrderBookV14.bidOffers[price]!.amount, owner: OrderBookV14.bidOffers[price]!.maker)
                let userFlowVault = getAccount(self.maker).getCapability(/public/flowTokenReceiver)
                  .borrow<&{FungibleToken.Receiver}>()!
                userFlowVault.deposit(from: <- contractFlowVault)
              }
              ...
              // bid order has equal FUSD amount for this order
                  ...
                  // transfer Flow from contract's to this order
                  let contractFlowVault <- FlowFusdVaultV4.withdrawFlow(amount: price*amount, owner: OrderBookV14.bidOffers[price]!.maker)
                  let userFlowVault = getAccount(self.maker).getCapability(/public/flowTokenReceiver)
                    .borrow<&{FungibleToken.Receiver}>()!
                  userFlowVault.deposit(from: <- contractFlowVault)
              }
